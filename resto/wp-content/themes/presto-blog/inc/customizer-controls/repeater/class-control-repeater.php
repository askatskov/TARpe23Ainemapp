...
if( ! class_exists( 'Presto_Blog_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Presto_Blog_Control_Repeater extends WP_Customize_Control {
    	 ...
    	 * @var string
    	 */
    	public $type = 'presto-blog-repeater';
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'presto-blog' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'presto-blog' ), $this->row_label['value'] );
    		}
    	...
    	public function enqueue() {    
            ...
            wp_enqueue_script( 'presto-blog-repeater', get_template_directory_uri() . '/inc/customizer-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'presto-blog-repeater', get_template_directory_uri() . '/inc/customizer-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<label>
    			<?php if ( ! empty( $this->label ) ) : ?>
    				<span class="customizerize-control-title"><?php echo esc_html( $this->label ); ?></span>
    			<?php endif; ?>
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'presto-blog' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'presto-blog' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="presto-blog-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'presto-blog' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_html_e( 'No Image Selected', 'presto-blog' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'presto-blog' ); ?></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add Image', 'presto-blog' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'presto-blog' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change Image', 'presto-blog' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add Image', 'presto-blog' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="presto-blog-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'presto-blog' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_html_e( 'No File Selected', 'presto-blog' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'presto-blog' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'presto-blog' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change File', 'presto-blog' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add File', 'presto-blog' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'presto-blog' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
