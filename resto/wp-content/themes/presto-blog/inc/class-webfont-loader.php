...
if ( ! class_exists( 'Presto_Blog_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Presto_Blog_WebFont_Loader {
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'presto_blog_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'presto_blog_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'presto_blog_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'presto_blog_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function load_preload_local_fonts( $url, $format = 'woff2' ) {
			...
			$local_font_files = get_site_option( 'presto_blog_local_font_files', false );
	
			if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
				$font_format = apply_filters( 'presto_blog_local_google_fonts_format', $format );
			...
			$font = presto_blog_webfont_loader_instance( $url );
		...
		public function delete_fonts_folder() {
			...
			delete_site_option( 'presto_blog_local_font_files' );
...
if ( ! function_exists( 'presto_blog_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function presto_blog_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Presto_Blog_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'presto_blog_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function presto_blog_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Presto_Blog_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
/**
 * Create instance of Presto_Blog_WebFont_Loader class.
 */
function presto_blog_webfont_loader_instance( $font_url = '' ) {
	return new Presto_Blog_WebFont_Loader( $font_url );
}
